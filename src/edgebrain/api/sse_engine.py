...
def create_event_stream(query: str) -> AsyncGenerator[str, None]:
    """
    创建 SSE 事件流生成器。
    该函数封装了 LangGraph 的状态流转逻辑，供 API 或测试调用。
    """
...
async def stream_status(q: str = Query(..., description="User query to process")):
    return StreamingResponse(create_event_stream(q), media_type="text/event-stream")
